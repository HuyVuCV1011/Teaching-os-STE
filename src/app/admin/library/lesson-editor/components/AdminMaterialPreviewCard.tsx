              ...
              <div className="border border-slate-800 bg-slate-955/20 rounded-2xl p-6 shadow-sm text-slate-300 flex justify-between items-center h-[100px]">
                ...
                {downloadAllowed && hasValidUrl ? (
                  <a
                    ...
                    className="px-3 py-1.5 rounded-lg bg-slate-800 hover:bg-slate-750 text-slate-200 font-semibold text-xs border border-slate-700 transition-colors flex items-center gap-1.5"
                  >
                    Download PDF
                  </a>
                ) : downloadAllowed ? (
                  <span className="text-xs text-slate-500 italic">File not found in storage</span>
                ) : (
                  <span className="text-xs text-slate-500 italic">Downloads disabled</span>
                )}
              </div>
            )}
          </div>
      ...
      {m.type === 'docx' && (() => {
        ...
        return (
          <div className="border border-slate-800 bg-slate-955/20 rounded-2xl p-6 shadow-sm text-slate-200 space-y-4 h-[450px] overflow-y-auto flex flex-col">
            <div className="flex justify-between items-center pb-3 border-b border-slate-800 shrink-0">
              ...
              {downloadAllowed && (
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="px-2.5 py-1 rounded bg-slate-800 hover:bg-slate-750 text-slate-200 font-semibold text-[10px] border border-slate-700 transition-colors whitespace-nowrap shrink-0"
                >
                  Download
                </a>
              )}
            ...
            {displayMode !== 'original' && (
              <div
                className="prose max-w-none text-slate-300 leading-relaxed text-xs flex-1 overflow-y-auto"
                dangerouslySetInnerHTML={{
                  __html: m.metadata?.viewer_artifact?.viewer_html || '<p class="text-slate-500 italic">No HTML preview available.</p>'
                }}
              />
            )}
          </div>
        ...
        return (
          <div className="border border-slate-800 bg-slate-955/20 rounded-2xl p-6 shadow-sm text-slate-200 space-y-4 h-[450px] overflow-y-auto flex flex-col">
            ...
            {displayMode !== 'original' && (
              <>
                {rows.length > 0 ? (
                  <div className="overflow-x-auto border border-slate-800 rounded-xl flex-1 overflow-y-auto">
                    <table className="min-w-full divide-y divide-slate-800 text-xs">
                      <thead className="bg-slate-900 sticky top-0 z-10">
                        <tr>
                          {headers.map((hdr: string, i: number) => (
                            <th key={i} className="px-3 py-2 text-left font-bold text-slate-200 border-r border-slate-800 last:border-0 whitespace-nowrap bg-slate-900">
                              {hdr}
                            </th>
                          ))}
                        </tr>
                  ...
                  <p className="text-xs text-slate-500 italic">No table data available.</p>
                )}
