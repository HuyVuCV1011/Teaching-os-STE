...
export function LessonInfoStep({
  ...
  const [showTheoryPreview, setShowTheoryPreview] = React.useState(false)
  ...
  const handleClearContent = () => {
    if (confirm('Bạn có chắc chắn muốn xóa giáo trình lý thuyết này?')) {
      setContent('')
      setIsDirty(true)
    }
  }
  return (
    <div className="space-y-6">
      ...
      {/* Card B: Giáo Trình Lý Thuyết (Theory Syllabus) */}
      <div className="bg-slate-900/10 border border-slate-700 p-6 rounded-2xl space-y-4">
        <div className="flex justify-between items-center pb-3 border-b border-slate-700">
          <div>
            <h3 className="text-sm font-bold text-slate-100 uppercase tracking-wider flex items-center gap-2">
              <BookOpen className="w-4 h-4 text-blue-500" />
              Giáo Trình Lý Thuyết (Syllabus/Textbook)
            </h3>
            <p className="text-[10px] text-slate-400 mt-0.5">
              Hệ thống chỉ nhận file `.md` (Markdown) do bạn tự chuẩn bị qua AI hoặc công cụ khác.
            </p>
          </div>
          <div>
            {content ? (
              <span className="bg-emerald-500/10 text-emerald-600 border border-emerald-500/20 px-2.5 py-1 rounded-full font-bold text-[9px] uppercase tracking-wider flex items-center gap-1">
                <CheckCircle className="w-3 h-3" />
                Đã có giáo trình
              </span>
            ) : (
              <span className="bg-slate-900 border border-slate-800 text-slate-500 px-2.5 py-1 rounded-full font-bold text-[9px] uppercase tracking-wider">
                Chưa có giáo trình
              </span>
            )}
          </div>
        ...
        <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center justify-between bg-slate-950/20 p-4 border border-slate-800 rounded-xl">
          ...
          <div className="flex gap-2 shrink-0">
            ...
            {content ? (
              <>
                <button
                  type="button"
                  onClick={() => setShowTheoryPreview(!showTheoryPreview)}
                  className="px-3 py-1.5 rounded-lg border border-slate-700 bg-slate-950 text-slate-450 hover:text-slate-200 hover:border-slate-550 text-xs font-semibold transition-all flex items-center gap-1"
                >
                  <Eye className="w-3.5 h-3.5" />
                  <span>{showTheoryPreview ? 'Ẩn xem trước' : 'Xem trước'}</span>
                </button>
                
                <label
                  htmlFor="theory-md-upload"
                  className="cursor-pointer px-3 py-1.5 rounded-lg border border-blue-800 bg-blue-800 hover:bg-blue-900 text-white text-xs font-semibold transition-all flex items-center gap-1"
                >
                ...
                <button
                  ...
                  className="px-3 py-1.5 rounded-lg border border-rose-900/30 bg-rose-500/10 hover:bg-rose-500/20 text-rose-600 text-xs font-semibold transition-all flex items-center gap-1"
                >
              ...
              <label
                htmlFor="theory-md-upload"
                className="cursor-pointer px-4 py-2 rounded-xl bg-blue-800 hover:bg-blue-900 text-white font-semibold text-xs transition-colors flex items-center gap-1.5 shadow-md focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-800/30"
              >
                <Upload className="w-3.5 h-3.5" />
                Tải lên File .md
              </label>
        ...
        {showTheoryPreview && content && (
          <div className="mt-4 p-5 border border-slate-800 bg-slate-950 rounded-xl max-h-96 overflow-y-auto custom-scrollbar shadow-inner">
            <h4 className="text-[10px] text-slate-500 font-bold uppercase tracking-wider mb-3 border-b border-slate-900/50 pb-2">
              Preview Giáo Trình (Rendered HTML)
            </h4>
            <article
