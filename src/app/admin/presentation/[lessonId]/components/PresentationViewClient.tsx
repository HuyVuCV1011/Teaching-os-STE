    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 flex flex-col font-sans">
      ...
      <main className="flex-1 flex overflow-hidden">
        ...
        <section className="flex-1 flex flex-col overflow-hidden bg-slate-950 border-r border-slate-800">
          ...
          {activeTab === 'theory' && (
            <div className="flex-1 overflow-y-auto p-5 md:p-8 custom-scrollbar bg-slate-950">
          ...
          {activeTab === 'assignments' && (
            <div className="flex-1 overflow-y-auto p-5 md:p-6 custom-scrollbar bg-slate-950">
                ...
                {assignments.length === 0 ? (
                  ...
                  <div className="space-y-4">
                    {assignments.map((assign, index) => (
                      <div key={assign.id} className="border border-slate-800 bg-slate-900 rounded-2xl p-4 md:p-5 space-y-4">
                        <div className="flex items-center justify-between pb-3 border-b border-slate-800">
                          ...
                          {assign.max_score && (
                            <span className="text-xs bg-slate-950 border border-slate-800 text-slate-400 px-2.5 py-1 rounded-lg font-bold font-mono">
                                  ...
                                  {questionsList.map((q: any, qIdx: number) => (
                                    <div key={q.id || qIdx} className="bg-slate-950 border border-slate-800/80 p-4 rounded-xl space-y-2.5 text-left shadow-sm">
                                      <div className="flex items-center justify-between pb-2 border-b border-slate-900">
                                        ...
                                        {q.points && (
                                          <span className="text-[11px] bg-blue-500/10 text-blue-400 border border-blue-500/20 px-2.5 py-0.5 rounded font-bold font-mono">
                                            {q.points} pts
                                          </span>
                                        )}
                                      </div>
                                      ...
                                      {q.options && q.options.length > 0 && (
                                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5 mt-2">
                                          {q.options.map((opt: string, optIdx: number) => (
                                            <div key={optIdx} className="p-3 rounded-lg border border-slate-850 bg-slate-900/40 text-xs text-slate-400 flex items-start gap-2 hover:border-slate-805 transition-all">
                                      ...
                                      {q.answer && (
                                        <div className="mt-2.5 p-3 bg-emerald-500/5 border border-emerald-500/20 rounded-xl text-slate-300 text-[11px] leading-relaxed">
                                          <div className="flex items-center gap-1.5 mb-1">
                                            <span className="w-1.5 h-1.5 rounded-full bg-emerald-500" />
                                            <strong className="text-emerald-600 text-sm font-bold">Đáp án & Hướng dẫn chấm:</strong>
                                          </div>
                                          <p className="pl-3 border-l border-emerald-500/20 text-slate-400 font-mono text-xs whitespace-pre-wrap">{q.answer}</p>
                                        </div>
                                      )}
                                    </div>
                                  ))}
        ...
        <aside className="w-80 bg-slate-900 flex flex-col overflow-y-auto custom-scrollbar shrink-0">
          ...
          <div className="p-5 border-b border-slate-800 space-y-4 shrink-0">
            ...
            {/* External Links List */}
            {(() => {
              const externalLinks = materials.filter(m => m.type === 'link')
              if (externalLinks.length === 0) return null
              return (
                <div className="space-y-2 pt-2">
                  <span className="text-xs font-medium text-slate-500">Liên kết tham khảo (Links)</span>
                  <div className="space-y-1.5">
                    {externalLinks.map((m) => (
                      <a
                        key={m.id}
                      ...
                      >
                        <span className="flex items-center gap-2 truncate">
                          <ExternalLink className="w-3.5 h-3.5 text-blue-600 group-hover:text-blue-500" />
                          <span className="truncate">{m.title}</span>
                        </span>
